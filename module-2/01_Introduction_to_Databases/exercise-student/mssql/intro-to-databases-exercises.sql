...
SELECT 
		name, governmentform, headofstate
FROM 
		country
WHERE 
		continent = 'Europe';
...
SELECT 
		name, population, surfacearea, lifeexpectancy
FROM 
		country
WHERE 
		continent = 'Asia';
...
SELECT 
		name, continent, population, gnp, lifeexpectancy
FROM 
		country
WHERE 
		lifeexpectancy > 80;
...
SELECT 
		name, continent, gnp, lifeexpectancy
FROM 
		country
WHERE 
		lifeexpectancy >= 70 
...
SELECT 
		(gnp / population) * (100000) AS 'per capita gnp (US $)', gnp, population, name, region
FROM 
		country
WHERE 
		continent = 'Europe';
...
SELECT 
		(2021 - indepyear) AS 'number of years since independence', indepyear, name
From 
		country
WHERE 
